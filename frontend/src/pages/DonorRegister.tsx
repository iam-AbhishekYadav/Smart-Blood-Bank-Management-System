...
import { Droplets, User, Mail, Phone, Lock, MapPin, HeartPulse } from "lucide-react";
...
const DonorRegister = () => {
  const [form, setForm] = useState({ name: "", email: "", phone: "", password: "", bloodGroup: "", age: "", address: "", healthConditions: "" });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-12 bg-muted">
      ...
      >
        ...
        <h1 className="text-2xl font-extrabold text-foreground mb-1">Donor Registration</h1>
        <p className="text-sm text-muted-foreground mb-8">Create your donor account and start saving lives.</p>
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input placeholder="John Doe" className="pl-10 h-11" value={form.name} onChange={(e) => update("name", e.target.value)} />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm font-medium">Age</Label>
              <Input type="number" placeholder="25" className="h-11" value={form.age} onChange={(e) => update("age", e.target.value)} />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input placeholder="Your full address" className="pl-10 h-11" value={form.address} onChange={(e) => update("address", e.target.value)} />
            </div>
          </div>

          <div className="space-y-1.5">
            <Label className="text-sm font-medium">Health Conditions <span className="text-muted-foreground">(optional)</span></Label>
            <div className="relative">
              <HeartPulse className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input placeholder="e.g. Diabetes, None" className="pl-10 h-11" value={form.healthConditions} onChange={(e) => update("healthConditions", e.target.value)} />
            </div>
          </div>
